...
import { CategoryList, SingleCategory } from "../models/games.model";
...
export class GamesService {
  ...
  getGames(category: string): Observable<SingleCategory> {
    if (!this.apiCache.games[category]) { // Cache it once if apiCache value is false
      this.apiCache.games[category] = this.http
        .get<SingleCategory>(`${this.apiUrl}/game-categories/${category}?brand=cherrycasino.desktop&locale=en`).pipe(
  ...
  getCategories(): Observable<CategoryList> {
    ...
    if (!this.apiCache.categories) {
      this.apiCache.categories = this.http
        .get<CategoryList>(`${this.apiUrl}/game-categories?brand=cherrycasino.desktop&locale=en`).pipe(
          publishReplay(1), // cache the latest emitted
          refCount(), // keep the Observable alive as long as there are any Subscribers
          retry(1),
          catchError(this.handleError)
